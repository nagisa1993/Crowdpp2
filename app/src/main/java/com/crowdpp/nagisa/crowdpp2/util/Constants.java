...
public class Constants {
    ...
    public static String ACCESS_KEY_ID = "AKIAIH67T52BR5JQTSAQ";
    public static String SECRET_KEY = "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";
    ...
    public static final String FILE_BUCKET = "file-bucket123";
    public static final String FILE_NAME = "NameOfThePicture";


    public static String getFileBucket() {
        return ("my-unique-name" + ACCESS_KEY_ID + FILE_BUCKET).toLowerCase(Locale.US);
    }

    public static String getDbBucket(){
        return ("my-database-name" + ACCESS_KEY_ID + FILE_BUCKET).toLowerCase(Locale.US);
    }
    ...
    public Constants(Context c) {
        ...
        PHONE_ID 	= c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("brand", "") + "_"
                + c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("model", "");

        PHONE_ID = PHONE_ID.replaceAll(" ", "");

        dbName	= PHONE_ID + ".db";

        mfcc_dist_same_semi = Double.parseDouble(c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("mfcc_dist_same_semi", ""));
        mfcc_dist_diff_semi = Double.parseDouble(c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("mfcc_dist_diff_semi", ""));
        mfcc_dist_same_un = Double.parseDouble(c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("mfcc_dist_same_un", ""));
        mfcc_dist_diff_un = Double.parseDouble(c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("mfcc_dist_diff_un", ""));
    }

    public static String getPhoneID(Context c){
        String Phone_ID 	= c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("brand", "") + "_"
                + c.getSharedPreferences("crowdpp", Context.MODE_PRIVATE).getString("model", "");
